...
dePair.binary <- function(df,dat1, dat2, padj.th=0.01, lfc.th=1, wilcox.th=NULL, q=NULL, q.b=NULL, q1=NULL, q2=NULL)
  {
    df = df[order(df$pval),]
    df$padj[df$padj < 10^-20]=10^-20
    select=with(df, padj < padj.th & abs(lfc)>lfc.th)
    ...
    if (!is.null(q)){
      if(is.null(q1)){
        q1=rowQuantiles(dat1[select,,drop=F], probs=c(q,q.b))
      }
      if(is.null(q2)){
        q2=rowQuantiles(dat2[select,,drop=F], probs=c(q,q.b))
      }
      if(length(select)>1){
        select1= select########## 
      }
...
deMatrix <- function(de.df, levels=NULL, padj.th=0.05, lfc.th=1)
  {
    de.genes = sapply(de.df, function(x){
      row.names(x)[abs(x$lfc)> lfc.th & x$padj < padj.th]
    },simplify=F)
    de.num = sapply(de.genes, length)
    pairs = do.call("rbind",strsplit(names(de.num),"_"))
    if(is.null(levels)){
      levels = sort(unique(as.vector(pairs)))
    }
    n.cl = length(levels)
    de.num.mat = matrix(0, nrow = n.cl, ncol=n.cl)
    row.names(de.num.mat) = colnames(de.num.mat) = levels
    for(i in 1:nrow(pairs)){
      de.num.mat[pairs[i,1], pairs[i,2]] = de.num[i]
      de.num.mat[pairs[i,2], pairs[i,1]] = de.num[i]
    }
    return(list(de.num.mat, de.genes))
  }
